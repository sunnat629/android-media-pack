...
- [ ] `DefaultDrmSessionManagerProvider` attached to `DefaultMediaSourceFactory`, not to any individual `MediaSource.Factory`.
- [ ] Each protected `MediaItem` has a `DrmConfiguration(C.WIDEVINE_UUID)` with a license URI and headers.
- [ ] `detectWidevineLevel()` is called before selecting high-resolution variants.
- [ ] Offline license lifecycle handled: `OfflineLicenseHelper.downloadLicense`, `renewLicense`, and `releaseLicense`.
- [ ] `Player.Listener.onPlayerError` handles every `ERROR_CODE_DRM_*` explicitly.
- [ ] HDCP-restricted policy surfaces a user-visible message on `ERROR_CODE_DRM_DISALLOWED_OPERATION`.
- [ ] License auth travels as `setLicenseRequestHeaders`, never in the query string.
- [ ] All license requests are HTTPS with a clear `User-Agent`.
- [ ] No `DrmSessionManager.Builder` usage anywhere in the codebase.
