...
- [ ] `minSdk` 21 or later, Media3 pinned to 1.9.0 or later.
- [ ] Manifest declares the service with `foregroundServiceType="mediaPlayback"` and the `androidx.media3.session.MediaSessionService` action.
- [ ] App holds `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MEDIA_PLAYBACK`, and requests `POST_NOTIFICATIONS` at runtime on API 33+.
- [ ] `PlaybackService` subclasses `MediaSessionService`, constructs exactly one `ExoPlayer`, and exposes it through one `MediaSession`.
- [ ] `AudioAttributes` set to `USAGE_MEDIA` plus `AUDIO_CONTENT_TYPE_MUSIC`, with `handleAudioFocus = true`.
- [ ] `setHandleAudioBecomingNoisy(true)` enabled.
- [ ] UI connects via `MediaController.Builder(context, SessionToken(...)).buildAsync()`. No direct `bindService` to the playback service.
- [ ] Notification drawer uses `setMediaButtonPreferences` with standard `Player.COMMAND_*` values.
- [ ] `onTaskRemoved` stops the service only when playback is paused or the queue is empty.
- [ ] `onDestroy` releases the `MediaSession` and the `ExoPlayer`.
- [ ] `Player.Listener.onPlayerError` handles `StuckPlayerException` explicitly and logs it.
- [ ] No manual `PowerManager.WakeLock` acquisition anywhere in the playback path.
- [ ] Device matrix (API 28, 33, 34, 35) passes all six scenarios from the validation plan.
