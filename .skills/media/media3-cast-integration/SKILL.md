...
- [ ] `media3-cast` and `play-services-cast-framework` on the classpath.
- [ ] `CastOptionsProvider` registered via `meta-data OPTIONS_PROVIDER_CLASS_NAME`.
- [ ] `CastPlayer.Builder(context).setLocalPlayer(exoPlayer).build()` is the single `Player` handed to `MediaSession.Builder`.
- [ ] No manual `session.setPlayer(...)` swap on Cast session start or end.
- [ ] UI exposes `MediaRouteButton` (Compose or XML).
- [ ] `SessionAvailabilityListener` logs session start and end.
- [ ] All `MediaItem` instances set `setMimeType` correctly.
- [ ] `onPlayerError` handles `ERROR_CODE_REMOTE_ERROR` and `ERROR_CODE_IO_NETWORK_CONNECTION_FAILED` distinctly.
- [ ] Manual smoke test: local playback, cast handoff, pause and resume on receiver, end session, confirm local resumes at the same position.
