...
- [ ] `media3-inspector` on the classpath.
- [ ] Every new call site uses `MetadataRetriever`, `FrameExtractor`, or `MediaExtractorCompat` instead of the platform `MediaMetadataRetriever` or `MediaExtractor`.
- [ ] All calls run on `Dispatchers.IO` or an equivalent background executor.
- [ ] Every retriever and extractor is closed with `.use { ... }`.
- [ ] Batch jobs use a bounded `Semaphore` (recommended 2 to 4 permits).
- [ ] Session metadata is populated from retriever output, not from hardcoded strings.
- [ ] Smoke test: local MP4, remote MP4, HLS VOD, progressive MP3. Confirm duration and thumbnail are returned within 500ms on a warm device.
