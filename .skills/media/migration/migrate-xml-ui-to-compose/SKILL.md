...
- Project **MUST NOT** keep both `PlayerView` (XML) and `PlayerSurface` (Compose) attached to the same `Player` instance at the same time.
- Project **MUST NOT** write custom `SurfaceView` subclasses.
...
3. Identify every custom player control. Each control translates to a Compose slot.
4. Confirm the player instance is a `MediaController` obtained from a `MediaSessionService`.
5. Plan the rollout: migrate one screen at a time.
...
Compose `PlayerControls` from `media3-ui-compose-material3` renders the subtitle toggle and selection sheet when the current item has text tracks.

## Step 7: remove AndroidView interop
...
1. Ship the Compose inline player first.
2. Ship the Compose fullscreen screen next.
3. Ship PiP and Cast last.
...
- **Both `PlayerView` and `PlayerSurface` attached to the same `Player`.**
- **Custom `SurfaceView` subclass.**
- **Manual system-UI fullscreen fights Compose `WindowInsetsController`.**
- **Leaving `controller_layout_id` XML after migration.**
- **Dropping `media3-ui` dependency while an XML screen still uses it.**
- **Missing `supportsPictureInPicture` in the manifest.**
- **Releasing the `MediaController` on a fullscreen toggle.**
- **Keeping the XML screen behind a flag forever.**
