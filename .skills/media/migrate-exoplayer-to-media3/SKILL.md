...
- [ ] Every `com.google.android.exoplayer2` import replaced with the correct `androidx.media3.*` package.
- [ ] Media3 dependencies pinned to 1.9.0 or later.
- [ ] `SimpleExoPlayer` replaced with `ExoPlayer`.
- [ ] `StyledPlayerView` replaced with `PlayerView`.
- [ ] `MediaSessionConnector` replaced with `androidx.media3.session.MediaSession` inside a `MediaSessionService`.
- [ ] `PlayerNotificationManager` removed.
- [ ] DRM wired through `DefaultDrmSessionManagerProvider` on `DefaultMediaSourceFactory`.
- [ ] Manifest declares the service action and `foregroundServiceType="mediaPlayback"`.
- [ ] Cast integration uses `CastPlayer.Builder(context).setLocalPlayer(exoPlayer)`.
- [ ] Media button drawer uses `setMediaButtonPreferences` with standard `Player.COMMAND_*` values.
- [ ] Manual wake lock code removed.
- [ ] Volume caching for mute replaced with `player.mute()` / `player.unmute()`.
- [ ] `StuckPlayerException` logged in analytics.
- [ ] ProGuard / R8 keep rules updated.
- [ ] `./gradlew build` succeeds and manual smoke test passes.
